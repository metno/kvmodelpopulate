...
def _progress_indicator(max_count):
    ...
    while True:
        nextLine = "\r%s [%d%%] - %d/%d    " % (
            progress_list[index % entries],
            (float(index) / max_count) * 100,
            index,
            max_count,
        )
        yield nextLine
...
def extractWantedData(forecast, from_first_time):
    ...
    try:
        ...
        log.warn(
            "Error when creating RR data aggregates: object was not reported from yr: "
            + str(e),
        )
    ...
    try:
        ...
        for t in range(3, 13, 3):
            ...
            if last_time in forecast:
                ret[working_time]["PP"] = (
                    forecast[last_time]["PR"] - forecast[working_time]["PR"]
                )
            last_time = working_time
    except KeyError as e:
        log.warn(
            "Error when creating PP data aggregates: object was not reported from yr: "
            + str(e),
        )
...
def populate_kvalobs(options):
    performanceYrSum = 0.0
    performanceDbSum = 0.0
    performanceTot = 0.0
    performanceCount = 0
    collectStart = time.perf_counter()
    nOk = 0
    ...
    log.info(
        f"Fetching model data from yr to kvalobs. User-Agent: {options.user_agent}"
    )
    ...
    if stations_without_coordinates:
        for s in stations_without_coordinates:
            del stations[s]
            log.debug(
                f"Ignoring station {station}, since we haven't got its coordinates" 
            )

    log.info(f"Fetching model data for {len(stations)} stations")
    ...
    for i in range(3):
        ...
        for station in timed_yielder(sorted_stations, options.sleep):
            ...
            try:
                startYr = time.perf_counter()
                locationforecast = yr.getLocationForecast(location, options.user_agent)
                endYr = time.perf_counter()
                nOk += 1
            except:
                log.warning(f"failed attempt station {station}")
            ...
            forecast = extractWantedData(locationforecast, options.from_first_time)
            startDb = time.perf_counter()
            connection.save_model_data(station, forecast)
            endDb = time.perf_counter()
            performanceDbSum += endDb - startDb
            performanceYrSum += endYr - startYr
            performanceTot += endDb - startYr
            performanceCount += 1
            log.debug(
                f"Station {station} performance: yr {endYr-startYr:.3f}s, db {endDb-startDb:.3f}s, total {endDb-startYr:.3f}s"
            )
        if not failed:
            break
    ...
    collectionTime = int(time.perf_counter() - collectStart)
    nStations = len(stations)
    if nOk == nStations:
        status_log.info("SUMARY %d/%d, fetched all stations" % (nOk, nStations))
    elif nOk == 0:
        status_log.error(
            "SUMMARY %d/%d, failed to fetch any stations" % (nOk, nStations)
        )
    else:
        status_log.warning(
            "SUMMARY %d/%d, failed to get some stations" % (nOk, nStations)
        )

    hours = int(collectionTime / 3600)
    minutes = int((collectionTime % 3600) / 60)
    seconds = int((collectionTime % 3600) % 60)
    status_log.info(
        f"SUMMARY: Collected {performanceCount} stations in {hours}h {minutes}m {seconds}s"
    )
    if performanceCount > 0:
        status_log.info(
            f"SUMMARY: Performance (avarage) #stations {performanceCount}: yr {performanceYrSum/performanceCount:.3f}s, db: {performanceDbSum/performanceCount:.3f}s, total {performanceTot/performanceCount:.3f}s"
        )
...
def main():
    ...
    database_options.add_option(
        "-p", "--port", type="int", metavar="PORT", help="database server port"
    )
    database_options.add_option(
        "-U", "--username", metavar="USERNAME", dest="user", help="database user name"
    )
    database_options.add_option(
        "", "--password", metavar="PASSWORD", dest="password", help="database password"
    )
    ...
    parser.add_option(
        ...
        "--user-agent",
        action="store_true",
        default="kvalobs (borgem@met.no)",
        dest="user_agent",
        help="Use this as the user agent when calling api.met.no",
    )
    ...
    try:
        ...
        print(
            "Invalid value for log-level: <%s>. Using <info> instead."
            % (options.log_level,),
            file=sys.stderr,
        )
    ...
    if options.statusfile:
        ...
        status_log.addHandler(stderrLogger)
        status_log.setLevel(LEVELS[options.log_level])
